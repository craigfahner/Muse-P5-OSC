...
function draw() {
  ...
  sendOsc('/battery2', Math.floor(batteryLevel));
  
  textSize(12);
  text('DELTA: ' + eeg.delta.toFixed(0), 10, 30);
  sendOsc('/delta2', int(eeg.delta.toFixed(0), 10, 30));
  text('THETA: ' + eeg.theta.toFixed(0), 10, 45);
  sendOsc('/theta2', int(eeg.theta.toFixed(0), 10, 45));
  text('ALPHA: ' + eeg.alpha.toFixed(0), 10, 60);
  sendOsc('/alpha2', int(eeg.alpha.toFixed(0), 10, 60));
  text('BETA:  ' + eeg.beta.toFixed(0),  10, 75);
  sendOsc('/beta2', int(eeg.beta.toFixed(0),  10, 75));
  text('GAMMA: ' + eeg.gamma.toFixed(0), 10, 90);
  sendOsc('/gamma2', int(eeg.gamma.toFixed(0), 10, 90));

  if (ppg.heartbeat) {
    text('HEART bpm: ' + ppg.bpm + ' •', 10, 120);
    sendOsc('/heartbeat2', 1);
  } else {
    sendOsc('/heartbeat2', 0);
    text('HEART bpm: ' + ppg.bpm, 10, 120);
  }

  sendOsc('/heartbpm2', int(ppg.bpm));

 
  
  text('ACCEL X: ' + accel.x.toFixed(2), 10, 150);
  sendOsc('/accel_x2', float(accel.x.toFixed(2)));
  text('ACCEL Y: ' + accel.y.toFixed(2), 10, 165);
  sendOsc('/accel_y2', float(accel.y.toFixed(2)));
  text('ACCEL Z: ' + accel.z.toFixed(2), 10, 180);
  sendOsc('/accel_z2', float(accel.z.toFixed(2)));

  text('GYRO X: ' + gyro.x.toFixed(2), 10, 210);
  sendOsc('/gyro_x2', float(gyro.x.toFixed(2)));
  text('GYRO Y: ' + gyro.y.toFixed(2), 10, 225);
  sendOsc('/gyro_y2', float(gyro.y.toFixed(2)));
  text('GYRO Z: ' + gyro.z.toFixed(2), 10, 240);
  sendOsc('/gyro_z2', float(gyro.z.toFixed(2)));
